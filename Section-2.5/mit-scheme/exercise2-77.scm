...
;; type-tags <== (map type-tag (list (cons 'rectangular (cons 3 4))))
...
(apply magnitude-rect (map contents (list (cons 'rectangular (cons 3 4)))))
(apply magnitude-rect (list (cons 3 4)))
(magnitude-rect (cons 3 4))
((lambda (z)
   (define (real-part) (car z))
   (define (imag-part) (cdr z))
   (sqrt (+ (square (real-part))
